...
ipcMain.handle('auth-register', async (_e, { username, password }) => {
  try {
    ...
    return { success: true, username: username.trim(), clearanceLevel: isFirstUser ? 4 : 2 }
  } catch (e) {
...
ipcMain.handle('auth-login-password', async (_e, { password }) => {
  // We require username OR just password (single-user-like flow)
  // For multi-user: pass username too. Here we try all users (single-user typical)
  try {
    ...
    // If single user, auto-select; multi-user requires username field
    const user = users[0]  // Could extend to pass username
